#!/usr/bin/env python
"""Analyze completed CE translation-ranking outputs.
...
def coverage_from_glossary(source: str, candidate: str, glossary: dict[str, list[str]]) -> float:
    present_terms = [term for term in glossary if _contains_source_term(source, term)]
    if not present_terms:
        return 1.0
    lower_candidate = candidate.lower()
    matched = 0
...
def fluency_proxy(candidate: str) -> float:
    ...
    chinese_chars = len(re.findall(r"[\u4e00-\u9fff]", candidate))
    mojibake_chars = len(re.findall(r"[�鈥€俓閴鍥藉]", candidate))
    quote_penalty = 0.06 if candidate.count('"') % 2 else 0.0
    repeated_punct_penalty = min(len(re.findall(r"([,.!?;:])\1+", candidate)) * 0.04, 0.2)
    avg_word_len = sum(len(word) for word in words) / max(len(words), 1)
    word_len_penalty = min(abs(avg_word_len - 5.2) / 10, 0.2)
    ascii_score = min(ascii_letters / max(length, 1), 1.0)
    score = ascii_score - min(chinese_chars / max(length, 1), 0.35) - min(mojibake_chars / max(length, 1), 0.5)
    score -= quote_penalty + repeated_punct_penalty + word_len_penalty
    return round(clamp(score), 6)
...
def candidate_features(row: dict[str, Any], letter: str) -> dict[str, float]:
    source = str(row["source_text"])
    candidate = str(row[f"candidate_{letter}"])
    source_units = max(len(re.findall(r"[\u4e00-\u9fff]", source)), 1)
    target_words = len(re.findall(r"[A-Za-z]+(?:[-'][A-Za-z]+)?", candidate))
    length_chars = len(candidate)
    length_ratio = target_words / source_units
    number_score = multiset_overlap_ratio(numeric_tokens(source), numeric_tokens(candidate))
    entity_score = entity_preservation(source, candidate)
    term_score = coverage_from_glossary(source, candidate, TERM_GLOSSARY)
    length_fit = math.exp(-abs(math.log(max(length_ratio, 0.01) / 0.62)))
    fluency = fluency_proxy(candidate)
    semantic_proxy = (
        0.35 * number_score
        + 0.25 * entity_score
...
def main() -> int:
    ...
    write_json(OUT_DIR / "ecpcfe_200ce.five_models.pairwise_judgments.json", pairwise_rows)

    agreement_rows = pairwise_agreement(pairwise_rows)
    write_csv(
        OUT_DIR / "ecpcfe_200ce.five_models.pairwise_agreement.csv",
        agreement_rows,
        [
            "model_1",
            "model_2",
    ...
    write_json(OUT_DIR / "ecpcfe_200ce.with_candidates.features.json", enriched)

    top1_rows = average_feature_rows(rankings, features_by_id)
    write_csv(
        OUT_DIR / "ecpcfe_200ce.five_models.top1_average_features.csv",
        top1_rows,
    ...
    if args.include_logistic:
        ...
        write_csv(
            OUT_DIR / "ecpcfe_200ce.five_models.pairwise_logistic_feature_preferences.csv",
            logistic_rows,
            [
                "model",
                "feature",
    ...
    write_json(OUT_DIR / "ecpcfe_200ce.five_models.consensus_borda.json", consensus)
    write_csv(
        OUT_DIR / "ecpcfe_200ce.five_models.consensus_borda_summary.csv",
        [
            {
                "id": row["id"],
    ...
    write_csv(
        OUT_DIR / "ecpcfe_200ce.five_models.model_vs_consensus_agreement.csv",
    ...
    write_csv(
        OUT_DIR / "ecpcfe_200ce.five_models.top1_entropy_layers.csv",
        [
            {
                "id": row["id"],
    ...
    write_csv(
        OUT_DIR / "ecpcfe_200ce.five_models.top1_entropy_layer_summary.csv",
    ...
    write_json(OUT_DIR / "ecpcfe_200ce.five_models.condorcet_detail.json", condorcet_detail)
    write_csv(
        OUT_DIR / "ecpcfe_200ce.five_models.condorcet_summary.csv",
        condorcet_summary,
        ["metric", "A", "B", "C", "none"],
    )

    minority_events, minority_summary = minority_model_rows(rankings)
    write_json(OUT_DIR / "ecpcfe_200ce.five_models.minority_model_events.json", minority_events)
    write_csv(
        OUT_DIR / "ecpcfe_200ce.five_models.minority_model_summary.csv",
        minority_summary,
    ...
    write_json(OUT_DIR / "ecpcfe_200ce.five_models.high_disagreement_samples.json", high_disagreement)
    write_csv(
        OUT_DIR / "ecpcfe_200ce.five_models.high_disagreement_summary.csv",
        [
            {
                "id": row["id"],
