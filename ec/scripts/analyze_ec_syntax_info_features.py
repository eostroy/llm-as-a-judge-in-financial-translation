#!/usr/bin/env python
"""Analyze EC syntax and information-structure features with Stanza.
...
PASSIVE_WORDS = {"被", "由", "为", "受到", "遭到", "经", "让", "给"}
NOMINALIZATION_SUFFIXES = ("性", "化", "度", "率", "者", "方", "额", "量", "值", "力")
PUNCT_CLAUSE_SPLIT = re.compile(r"[。！？!?；;]")
...
def stanza_features(nlp: Any, source: str, candidate: str) -> dict[str, float]:
    ...
    punct_clause_count = len([piece for piece in PUNCT_CLAUSE_SPLIT.split(candidate) if piece.strip()])
    ...
    coordination_count = sum(1 for word in words if word.deprel in {"cc", "conj"}) + sum(
        candidate.count(word) for word in COORDINATION_WORDS
    )

    passive_or_beishi_count = sum(candidate.count(word) for word in PASSIVE_WORDS) + sum(
        1 for word in words if "pass" in str(word.deprel).lower()
    )

    sentence_compression_ratio = chinese_char_count(candidate) / source_word_count(source)
...
def compute_features(dataset_rows: list[dict[str, Any]], cache_path: Path) -> tuple[list[dict[str, Any]], dict[str, dict[str, dict[str, float]]]]:
...
def feature_rank_rows(top1_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rows = []
    lower_is_more = {"passive_or_beishi_count"}
    for key in FEATURE_KEYS:
        metric = f"avg_{key}"
...
def main() -> int:
    ...
    print(f"wrote syntax/information-structure analysis for {len(dataset_rows)} samples")
    return 0
