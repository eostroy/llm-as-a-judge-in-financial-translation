#!/usr/bin/env python
"""Compare EC model-selected translations with FFN human translations syntactically."""
...
EC_SCRIPTS = ROOT / "ec" / "scripts"
if str(EC_SCRIPTS) not in sys.path:
    sys.path.insert(0, str(EC_SCRIPTS))

from analyze_ec_deep_features import CANDIDATES, MODEL_FILES, read_json, top_candidate, write_csv, write_json  # noqa: E402
from analyze_ec_syntax_info_features import FEATURE_KEYS, stanza_features  # noqa: E402


RAW_FFN = ROOT / "data/raw/ffn_200ec.csv"
DATASET = ROOT / "ec/datasets/ffn_200ec.with_candidates.shuffled.json"
CANDIDATE_SYNTAX = (
    ROOT / "ec/results/parser_derived_syntactic_metrics/analysis/pilot/json/ffn_200ec.syntax_info_features.by_candidate.json"
)
OUT_DIR = ROOT / "ec/results/parser_derived_syntactic_metrics/analysis/human_reference"
OUT_JSON_DIR = OUT_DIR / "json"
OUT_CSV_DIR = OUT_DIR / "csv"
CACHE_PATH = OUT_JSON_DIR / "ffn_200ec.human_translation.syntax_info_features.cache.json"
...
def compute_human_features(human_by_id: dict[str, dict[str, str]]) -> dict[str, dict[str, float]]:
    ...
    if missing:
        nlp = stanza.Pipeline("zh", processors="tokenize,pos,lemma,depparse", verbose=False, use_gpu=False)
...
def main() -> int:
    ...
    write_json(OUT_JSON_DIR / "ffn_200ec.human_translation.syntax_info_features.by_sample.json", enriched_human)
    write_json(OUT_JSON_DIR / "ffn_200ec.model_top1_vs_human.syntax_info_features.by_sample.json", pairs)
    write_csv(
        OUT_CSV_DIR / "ffn_200ec.model_top1_vs_human.syntax_info_features.summary.csv",
        summaries,
        [
            "model",
            "feature",
    ...
    print(f"wrote EC human-vs-model syntax comparison for {len(human_by_id)} samples and {len(rankings)} models")
    return 0
